...
class eF(tk.Frame):
    def __init__(self, parent, topOne):
        ...
        def loadValues():
            if(self.alphaCheckboxVariable.get() == 0):
                self.alpha = -1
            if(self.betaCheckboxVariable.get() == 0):
                self.beta = -1
            if(self.gammaCheckboxVariable.get() == 0):
                self.gamma = -1
            if(self.deltaCheckboxVariable.get() == 0):
                self.delta = -1
            self.topGUI.loadValues(self.alpha, self.beta, self.gamma, self.delta)
        ...
        def saveResults():
            stringToSave = "CommentSpace\n\n---------------------\n"
            for name in self.fileListbox.get(0, tk.END):
                stringToSave += name + "\n"
            stringToSave += "Parameter\tValue\tStd. Dev.\n"
            for child in self.resultsView.get_children():
                ...
                stringToSave += resultName + "\t" + resultValue + "\t" + resultStdDev + "\n"
            ...
            stringToSave += "Alpha\t" + str(correl[0][0]) + "\t" + str(correl[0][1]) + "\t" + str(correl[0][2]) + "\t" + str(correl[0][3])
        ...
        self.saveResultsButton = ttk.Button(self.resultsFrame, text="Save values and std. devs.", command=saveResults, width=25)
        ...
        self.saveResultsButton.grid(column=0, row=1, pady=3, sticky="E")
