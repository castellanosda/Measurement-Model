...
class myGUI:
    ...
    def __init__(self, master, args):
        ...
        self.currentDirectory = "/"              #The current directory; used in various other classes, but not a default setting
        self.defaultDirectory = "/"              #The default directory
        self.defaultFormulaDirectory = "/"       #The default directory for .mmformula files
        ...
        master.geometry('{}x{}'.format(1000, 830))               #GUI size
        master.iconbitmap(resource_path('img/elephant3.ico'))   #GUI icon

        img = ImageTk.PhotoImage(Image.open(resource_path("./img/elephant.png")))
    ...
    def loadValues(self, a, b, g, d):
        self.model_frame.errorAlphaCheckboxVariable.set(0)
        self.model_frame.errorBetaCheckboxVariable.set(0)
        self.model_frame.errorGammaCheckboxVariable.set(0)
        self.model_frame.errorDeltaCheckboxVariable.set(0)

        self.formula_frame.errorAlphaCheckboxVariable.set(0)
        self.formula_frame.errorBetaCheckboxVariable.set(0)
        self.formula_frame.errorGammaCheckboxVariable.set(0)
        self.formula_frame.errorDeltaCheckboxVariable.set(0)

        if(a != -1):
            self.model_frame.errorAlphaCheckboxVariable.set(1)
            self.model_frame.errorAlphaEntry.configure(state="normal")
            self.model_frame.errorAlphaVariable.set(a)
        
        if(b != -1):
            self.model_frame.errorBetaCheckboxVariable.set(1)
            self.model_frame.errorBetaEntry.configure(state="normal")
            self.model_frame.errorBetaVariable.set(b)
        
        if(g != -1):
            self.model_frame.errorGammaCheckboxVariable.set(1)
            self.model_frame.errorGammaEntry.configure(state="normal")
            self.model_frame.errorGammaVariable.set(g)
        
        if(d != -1):
            self.model_frame.errorDeltaCheckboxVariable.set(1)
            self.model_frame.errorDeltaEntry.configure(state="normal")
            self.model_frame.errorDeltaVariable.set(d)
        
        if(a != -1):
            self.formula_frame.errorAlphaCheckboxVariable.set(1)
            self.formula_frame.errorAlphaEntry.configure(state="normal")
            self.formula_frame.errorAlphaVariable.set(a)
        
        if(b != -1):
            self.formula_frame.errorBetaCheckboxVariable.set(1)
            self.formula_frame.errorBetaEntry.configure(state="normal")
            self.formula_frame.errorBetaVariable.set(b)
        
        if(g != -1):
            self.formula_frame.errorGammaCheckboxVariable.set(1)
            self.formula_frame.errorGammaEntry.configure(state="normal")
            self.formula_frame.errorGammaVariable.set(g)
        
        if(d != -1):
            self.formula_frame.errorDeltaCheckboxVariable.set(1)
            self.formula_frame.errorDeltaEntry.configure(state="normal")
            self.formula_frame.errorDeltaVariable.set(d)
