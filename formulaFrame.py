...
class fF(tk.Frame):
    def __init__(self, parent, topOne):
        ...
        def loadErrStruct():
            #Unmark boxes and set defaults
            self.errorAlphaCheckboxVariable.set(0)
            self.errorBetaCheckboxVariable.set(0)
            self.errorGammaCheckboxVariable.set(0)
            self.errorDeltaCheckboxVariable.set(0)
            self.errorAlphaCheckboxVariable.set(0)
            self.errorAlphaVariable.set(0.1)
            self.errorBetaVariable.set(0.1)
            self.errorGammaVariable.set(0.1)
        ...
        def saveAll():
            ...
            for i in range(len(self.wdata)):
                ...
                try:
                    #values for the columns above starting at SigmaZrconf
                    rCI = max(self.realFit[i] + 2*self.sdrReal[i], self.realFit[i] - 2*self.sdrReal[i]) - self.realFit[i]
                    jCI = max(self.imagFit[i] + 2*self.sdrImag[i], self.imagFit[i] - 2*self.sdrImag[i]) - self.imagFit[i]
                    stringToSave += "\t"  + str(self.sdrReal[i])\
                            + "\t" + str(self.sdrImag[i])\
                            + "\t" + str(self.fitWeightR[i])\
                            + "\t" + str(self.fitWeightJ[i])\
                            + "\t" + str(rCI)\
                            + "\t" + str(-1*rCI)\
                            + "\t" + str(jCI)\
                            + "\t" + str(-1*jCI)\
                            + "\t" + str(np.arctan2(self.jdata[i], self.rdata[i]) * (180/np.pi))\
                            + "\t" + str(np.arctan2(self.imagFit[i], self.realFit[i]) * (180/np.pi))\
                            + "\t" + str(np.sqrt(self.imagFit[i]**2 + self.realFit[i]**2))\
                            + "\t" + str((self.rdata[i] - self.realFit[i])/self.rdata[i])\
                            + "\t" + str((self.jdata[i] - self.imagFit[i])/self.jdata[i])\
                            + "\t" + str(rCI/self.rdata[i])\
                            + "\t" + str(-1*rCI/self.rdata[i])\
                            + "\t" + str(jCI/self.jdata[i])\
                            + "\t" + str(-1*jCI/self.jdata[i])\
                            + "\t" + str(2*self.fitWeightR[i]/self.rdata[i])\
                            + "\t" + str(-2*self.fitWeightR[i]/self.rdata[i])\
                            + "\t" + str(2*self.fitWeightJ[i]/self.jdata[i])\
                            + "\t" + str(-2*self.fitWeightJ[i]/self.jdata[i])\
                            + "\n"
                except Exception:
                    stringToSave += "\n"
        ...
        #Button for loading error structures
        self.errorStructureButton = ttk.Button(self.errorStructureFrame, text='Load Error Structure', command=loadErrStruct)
        self.errorStructureButton.grid(column=10, row=0, sticky="W")
